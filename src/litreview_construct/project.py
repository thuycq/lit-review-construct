...
def init_project(root: Path, name: str | None = None) -> dict[str, object]:
    ...
    event = {
        ...
        "model": None,
        "inputs": {},
        "outputs": [".litreview/project.yaml", ".litreview/state.json", "AGENTS.md"],
        "source_ids": [],
        "notes": None,
    }
    ...
    _atomic_write_text(
        state_root / "activity" / "activity.jsonl",
    ...
    return {"created": True, "root": str(root), "project_id": project_id}
...
def read_status(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "stage_status": state["stages"][state["current_stage"]]["status"],
...
def doctor(root: Path) -> list[dict[str, str]]:
    ...
    checks.append(
        {
            "check": "project_state",
            "status": "OK" if state_root.is_dir() else "WARNING",
            "detail": str(state_root),
        }
    )

    for filename in ["project.yaml", "state.json"]:
        path = state_root / filename
        checks.append(
            {
                "check": filename,
                "status": "OK" if path.is_file() else "WARNING",
                "detail": str(path),
            }
        )
