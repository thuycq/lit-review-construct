...
def prepare_final_landscape_packet(
    ...
    *,
    ...
    campaign = _completed_campaign(root)
    ...
    relations = _load_jsonl(root / PROJECT_DIR / "data" / "paper_relations.jsonl")
    relations = [
        row
        for row in relations
        if str(row.get("left_paper_id")) in selected_ids
        and str(row.get("right_paper_id")) in selected_ids
    ]
