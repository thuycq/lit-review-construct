Use Lit Review Construct in the current workspace.

If `.litreview/project.yaml` exists, resume from authoritative local state by running `lrc next . --json` and follow the returned Lit Review Construct skill/action. Do not reconstruct project state from chat history.

If `.litreview/project.yaml` does not exist, start a new Lit Review Construct project only because the researcher explicitly invoked this command. Initialize the workspace, then help the researcher complete Research Intent before discovery.
